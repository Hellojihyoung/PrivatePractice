...
// http://localhost:3000/user/findId/01045562725

// http://localhost:3000/user/findId
// {
// 	"phone_number": "01045562725",
// 	"certification_number": "940335"
// }
...
func getId(c echo.Context) error{ 
	...
	if compareCertificationNumber(params["phone_number"], params["certification_number"]){
		if idExist(params["phone_number"]){
	...
	err = db.QueryRow("SELECT login_id FROM user WHERE phone_number = ?", params["phone_number"]).Scan(&login_id)
...
func main() {
	...
	e.GET("/user/findId", getId) // 아이디 주기
	e.POST("/user/findId/:phoneNum", createAuthNum) // 전화번호 입력
